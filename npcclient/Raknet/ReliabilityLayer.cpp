...
InternalPacket * ReliabilityLayer::BuildPacketFromSplitPacketList( SplitPacketChannel *splitPacketChannel, CCTimeType time )
{
	...
	BitSize_t offset = 0; int k = 0;
	for (j=0; j < splitPacketChannel->splitPacketList.Size(); j++)
	{
		k = splitPacketChannel->splitPacketList[j]->splitPacketIndex;
		int _offset = 0;
		for (int m = 0; m < k; m++)
			_offset += splitPacketChannel->splitPacketList[m]->dataBitLength;
		splitPacket=splitPacketChannel->splitPacketList[j];
        memcpy(internalPacket->data + BITS_TO_BYTES(_offset), splitPacket->data, (size_t)BITS_TO_BYTES(splitPacketChannel->splitPacketList[j]->dataBitLength));
	...
	RakNet::OP_DELETE(splitPacketChannel, __FILE__, __LINE__);
	/*for (int n = 0; n < BITS_TO_BYTES(internalPacket->dataBitLength); n++)
	{
		printf("%x ", internalPacket->data[n]);
		if ((n + 1) % 16 == 0)printf("\n");
	}*/
	return internalPacket;
#endif
