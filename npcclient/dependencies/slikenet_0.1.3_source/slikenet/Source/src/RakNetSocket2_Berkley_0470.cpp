...
extern bool con_req_reply2_recvd;
void RNS2_Berkley::RecvFromBlockingIPV4(RNS2RecvStruct *recvFromStruct)
{
	...
	if (recvFromStruct->bytesRead >= 10 && con_req_reply2_recvd) //4+4+atleast 2
	{
		//Then decrypt the packet.
		unsigned char* data = (unsigned char*)recvFromStruct->data;
		uint32_t len; 
		memcpy(&len, data, 4);
		assert(sizeof(long) == 4);
		//printf("bytesRead is %d.\n", recvFromStruct->bytesRead);
		len = _byteswap_ulong(len) * 2;
		len+=(data[4] & 0x80)>>7;
		//printf("The length of the original RakNet packet was %d\n", len);
		//len is the length of original raknet packet
		//now, read next 3 bytes.
		uint32_t lenCompressedData = (data[7] + 256 * data[6] + 256 * 256 * data[5]+256*256*256*(data[4] & 127)) * 4;
		lenCompressedData += (data[8] & 0xc0) >> 6;//the remainder.
		//printf("Length of compressed data must be %d.\n", lenCompressedData);
		if (lenCompressedData + 9 > sizeof(recvFromStruct->data))
		{
			printf("lenCompressedData > recvData\n");
			exit(0);
		}
		//for(int i=0;i<recvFromStruct->bytesRead;i++)
		uint8_t* newdata = (uint8_t*)calloc(lenCompressedData, sizeof(uint8_t));
		if (!newdata)
		{
			printf("Error during memory allocation for vcmp decompression\n");
			exit(0);
		}
		//byte shift vcmp data 
		for (int i = 0; i < lenCompressedData; i++)
		{
			newdata[i] = (data[8 + i] & 63) << 2;
			//if (i + 1 < lenCompressedData)
			if (9 + i < recvFromStruct->bytesRead)
			{
				newdata[i] |= (data[9 + i] & 0xc0) >> 6;
			}
			//printf("%x,", newdata[i]);
		}
		//printf("\n");
		assert(sizeof(uLongf) == sizeof(uint32_t));
		uint32_t lenDecompressedData;
		int ret=uncompress(data, (uLong*)&lenDecompressedData, newdata, lenCompressedData);
		if (ret != Z_OK)
		{
			if (ret == Z_DATA_ERROR)
				printf("Decompression error: Corrupted data\n");
			else printf("Decompression error. Code %d\n", ret);
			exit(0);
		}
		/*for (int i = 0; i < lenDecompressedData; i++)
			printf("%x,", data[i]);
		printf("\n");*/
		recvFromStruct->bytesRead = lenDecompressedData;
	}
	recvFromStruct->timeRead= SLNet::GetTimeUS();
