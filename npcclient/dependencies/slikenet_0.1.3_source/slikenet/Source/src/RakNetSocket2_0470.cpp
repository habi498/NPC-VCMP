...
#include "RakNetSocket2_Berkley_0470.cpp"
...
//Whether we received OPEN_CONNECTION_REPLY_2 from VCMP Server
bool con_req_reply2_recvd = false;

RNS2SendResult RNS2_Windows::Send( RNS2_SendParameters *sendParameters, const char *file, unsigned int line ) {
	RNS2_SendParameters sendParameterNew = {};
	uint8_t* newdata; bool dataAllocated = false;
	if (con_req_reply2_recvd)
	{
		//do compression
		uint32_t len = sendParameters->length;
		newdata = (uint8_t*)calloc(8, sizeof(char));
		if (newdata)
		{
			dataAllocated = true; 
			uint32_t halflen = len / 2;
			assert(sizeof(long) == 4);
			uint32_t dw_swapped = _byteswap_ulong(halflen);
			memcpy(newdata, &dw_swapped, 4);
			uint32_t dw_block = 0;
			if (len % 2)dw_block |= 0x80000000;
			uint8_t* temp;
			temp = (uint8_t*)realloc(newdata, 8 + len + 6 + 1);//zlib has 6=2+4 as header and checksum. +1 because vcmp shifts bytes
			if (temp)
			{
				//Success.
				newdata = temp;
				...
				uint8_t ret = compress2(newdata + 8, (uLongf*)&outlen,
					(unsigned char*)sendParameters->data, sendParameters->length, Z_DEFAULT_COMPRESSION);
				if (ret == Z_OK)
				{
					//our compression was success. Now first complete dw_block.
					uint8_t outlenbyfour = outlen / 4;
					assert(outlenbyfour < 0x80000000);
					dw_block |= (outlenbyfour);
					assert(sizeof(long) == 4);
					uint32_t swapped = _byteswap_ulong(dw_block);
					memcpy(newdata + 4, &swapped, 4);
					uint8_t rem = outlen % 4;
					rem = (0x40) * (rem);
					uint8_t temp = 0;
					//now newdata+8 contains 78 9c ...
					for (int i = 0; i < outlen; i++)
					{
						temp = *(newdata + 8 + i) & 3;
						*(newdata + 8 + i) = *(newdata + 8 + i) >> 2;
						if (i == 0)*(newdata + 8 + i) |= rem;
						else *(newdata + 8 + i) |= rem;
						rem = temp << 6;
					}
					//One more byte left
					*(newdata + 8 + outlen) = rem;

					sendParameterNew.data = (char*)newdata;
					sendParameterNew.length = 8 + outlen + 1;
					sendParameterNew.systemAddress = sendParameters->systemAddress;
					sendParameterNew.ttl = sendParameters->ttl;
				}
			}
			else
			{
				printf("realloc failed"); exit(0);
			}

		}
		else {
			printf("Error during memory allocation for vcmp compression\n");
			exit(0);
		}
	}
	else sendParameterNew = *sendParameters;
	if (slo)
	{
		RNS2SendResult len;
		len = slo->RakNetSendTo(sendParameterNew.data, sendParameterNew.length, sendParameterNew.systemAddress);
		if (len >= 0)
		{
			if (dataAllocated)
				free(newdata);
	...
	RakNet::RNS2SendResult result= Send_Windows_Linux_360NoVDP(rns2Socket,&sendParameterNew, file, line);
	if (dataAllocated)
		free(newdata);//we allocated it 
	return result;
}
